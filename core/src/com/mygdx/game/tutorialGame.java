...
public class tutorialGame extends Game {
	...
	public void render() {
		...
		/*
		 * the movement of the game. Users can press: LEFT arrow key: to move left RIGHT
		 * arrow key: to move right D key: to accelerate
		 */

		float deltaTime = Gdx.graphics.getDeltaTime();
		if (Gdx.input.isKeyPressed(Keys.LEFT))
			bucket.x -= SPEED * deltaTime;
		if (Gdx.input.isKeyPressed(Keys.RIGHT))
			bucket.x += SPEED * deltaTime;

		if (Gdx.input.isKeyPressed(Keys.D) && !activated)
			powerActivated();
		if (activated && TimeUtils.nanoTime() - lastPowerTime > 2_100_000_000)
			powerDeactivated();

		// every interval spawn new raindrop or log
		if (TimeUtils.nanoTime() - lastDropTime > 1000000000)
			spawnRaindrop();
		if (TimeUtils.nanoTime() - lastLogTime > 2000000000)
			spawnLog();
...
//		// TESTING END
	...
	/*
	 * creates a new randomly positioned raindrops 
	 */
	...
	/*
	 * creates a new randomly positioned log 
	 */
